...
from app.models import Card, StudySession, Deck, User
...
def get_next_card(db: Session = Depends(get_db), deck_id: Optional[int] = None, current_user: User = Depends(require_user)):
    today = date.today()
...
def self_rate_answer(request: SelfRateRequest, db: Session = Depends(get_db), current_user: User = Depends(require_user)):
...
async def score_answer(request: ScoreAnswerRequest, db: Session = Depends(get_db), current_user: User = Depends(require_user)):
...
def get_study_stats(db: Session = Depends(get_db), deck_id: Optional[int] = None, current_user: User = Depends(require_user)):
    query = db.query(Card).filter(Card.user_id == current_user.id)
    if deck_id is not None:
        query = query.filter(Card.deck_id == deck_id)
...
@router.get("/study/scored-stats")
def get_scored_study_stats(db: Session = Depends(get_db), current_user: User = Depends(require_user)):
    sessions = db.query(StudySession).filter(StudySession.user_id == current_user.id).all()

    total = len(sessions)
    if total == 0:
        return {
            "total_answered": 0, "correct_count": 0, "partial_count": 0,
            "incorrect_count": 0, "average_score": 0.0
        }
    ...
    for s in sessions:
        if s.box_after >= 4:
            score = 9; correct_count += 1
        elif s.box_after >= 2:
            score = 6; partial_count += 1
        else:
            score = 2; incorrect_count += 1
        scores.append(score)

    avg_score = round(sum(scores) / len(scores), 1) if scores else 0.0

    return {
        "total_answered": total, "correct_count": correct_count,
        "partial_count": partial_count, "incorrect_count": incorrect_count,
        "average_score": avg_score
    }


@router.post("/study/clear-today")
def clear_today_stats(db: Session = Depends(get_db), current_user: User = Depends(require_user)):
    today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
...
def clear_all_stats(db: Session = Depends(get_db), current_user: User = Depends(require_user)):
    db.query(StudySession).filter(StudySession.user_id == current_user.id).delete()
    db.query(Card).filter(Card.user_id == current_user.id).update({
        "box": 1, "next_review_date": None, "last_reviewed": None
    })
    db.commit()
...
def clear_all_progress(db: Session = Depends(get_db), current_user: User = Depends(require_user)):
    from app.models import AIStudySession
    db.query(StudySession).filter(StudySession.user_id == current_user.id).delete()
    db.query(AIStudySession).filter(AIStudySession.user_id == current_user.id).delete()
    db.query(Card).filter(Card.user_id == current_user.id).update({
        "box": 1, "next_review_date": None, "last_reviewed": None
    })
    db.commit()
